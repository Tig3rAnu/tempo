...
export default async function PaymentsPage() {
  async function getPayments() {
    const data = await fs.readFile(
      path.join(
        process.cwd(),
        'app/(dashboard)/(routes)/payments/data/payments.json'
      )
    )
    const payments = JSON.parse(data.toString())
    return z.array(paymentSchema).parse(payments)
  }
  ...
  return (
    <div className="p-8 pt-6">
      <div className="flex justify-between items-center">
        ...
        <Button className="bg-[#FFB600] rounded-2xl px-4 py-2 text-black text-xl font-medium hidden">
          <Plus />
          Add New
        </Button>
      </div>
      <div className="pt-8">
        <DataTable data={payment} columns={columns} />
      </div>
    </div>
  )
}
